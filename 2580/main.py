...
def sudoku(row):
    if row == 9:
        ...
        exit(0)
    # 빈 숫자
    row_blank_nums = elements - set(board[row])
    col = board[row].index(0)
    col_blank_nums = set()
    for i in range(9):
        if board[i][col] != 0 and board[i][col] in col_blank_nums:
            return False
        col_blank_nums.add(board[i][col])

    blank_nums = row_blank_nums & col_blank_nums
    for i in blank_nums:
        board[row][col] = i
        if promising(row, col):
            if 0 not in board[row]:
